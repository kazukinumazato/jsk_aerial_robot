...
#ifndef __RADXA_COMPLEMENTARY_AHRS_H
#define __RADXA_COMPLEMENTARY_AHRS_H

#include <radxa/control/estimator.h>
...
class RadxaComplementaryAHRS: public RadxaEstimatorAlgorithm
{
public:
  RadxaComplementaryAHRS():RadxaEstimatorAlgorithm(), prev_mag_()
  {}

  radxa_ap::Quaternion getQuaternion()
  {
    radxa_ap::Quaternion q;
...
private:
  ...
  radxa_ap::Vector3f prev_mag_; /* for lpf filter method for mag */

  /* core esitmation process, using body frame */
  void estimation() 
  {
    RadxaEstimatorAlgorithm::estimation();

    int  valid_acc = 0;
    static int cnt = 0;

    float acc_magnitude = acc_ * acc_; //norm?
    radxa_ap::Vector3f est_g_b_tmp = est_g_;
    radxa_ap::Vector3f est_m_b_tmp = est_m_;

    radxa_ap::Vector3f gyro_rotate = gyro_  * DELTA_T;
    ...
    if (mag_ == radxa_ap::Vector3f())
      {
	est_m_ = radxa_ap::Vector3f(1, 0, 0);
      }
    
    radxa_ap::Vector3f wz_b = est_g_.normalized();
    radxa_ap::Vector3f wy_b = wz_b % est_m_;
    wy_b.normalize();
    radxa_ap::Vector3f wx_b = wy_b % wz_b;
    wx_b.normalize();

    radxa_ap::Matrix3f rot = radxa_ap::Matrix3f(wx_b, wy_b, wz_b); // rotation of body frame w.r.t. the world frame. Transpose from row to col
