...
#include <radxa/control/math/location.h>
...
float get_distance(const struct Location &loc1, const struct Location &loc2)
{
    ...
    float dlong             = ((float)(loc2.lng - loc1.lng)) * radxa_ap::longitude_scale(loc2);
...
{
    return radxa_ap::get_distance(loc1, loc2) * 100;
}
...
int32_t get_bearing_cd(const struct Location &loc1, const struct Location &loc2)
{
    ...
    int32_t off_y = (loc2.lat - loc1.lat) / radxa_ap::longitude_scale(loc2);
...
void location_offset(struct Location &loc, float ofs_north, float ofs_east)
{
    if (!is_zero(ofs_north) || !is_zero(ofs_east)) {
        ...
        int32_t dlng = (ofs_east * LOCATION_SCALING_FACTOR_INV) / radxa_ap::longitude_scale(loc);
        loc.lat += dlat;
        loc.lng += dlng;
    }
}
...
Vector2f location_diff(const struct Location &loc1, const struct Location &loc2)
{
    return Vector2f((loc2.lat - loc1.lat) * LOCATION_SCALING_FACTOR,
                    (loc2.lng - loc1.lng) * LOCATION_SCALING_FACTOR * radxa_ap::longitude_scale(loc1));
}
