...
class LemniTrajFollow:
    ...
    def __init__(self):
        ...
        self.period = float(rospy.get_param("~period", 30.0))     # [s] per lap
        self.a = float(rospy.get_param("~radius", 1.0))           # scale 半径
        self.init_phase = float(rospy.get_param("~init_theta", 0.0))  # [rad]
        self.center_x = float(rospy.get_param("~center_x", 0.0))
        self.center_y = float(rospy.get_param("~center_y", -0.5))
        self.loops = int(rospy.get_param("~loops", 3))            # number of laps
        ...
        # ---- Publisher ----
        self.nav_pub = rospy.Publisher("/crobat/uav/nav", FlightNav, queue_size=1)
